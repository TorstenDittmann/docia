...
async function writeTemplateFile(absolutePath: string, contents: string): Promise<void> {
...
export async function runInitCommand(context: CommandContext): Promise<number> {
	...
	const title = readStringFlag(context.flags, "title", "t") ?? toTitleCaseFromName(directoryName);
